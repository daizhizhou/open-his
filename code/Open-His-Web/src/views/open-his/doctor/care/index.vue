          ...
          <el-tabs v-model="careActiveName" type="card">
            ...
            <el-tab-pane label="处方" name="c2">
              ...
              <div style="margin:3px;text-align:left">
                <el-button type="success" icon="el-icon-plus" @click="handelAddMedicinesOrder">添加药用处方</el-button>
                <el-button type="success" icon="el-icon-plus" @click="handelAddCheckItemOrder">添加检查处方</el-button>
              </div>
            </el-tab-pane>
          </el-tabs></el-card>
        <!-- 右边的病历，处方结束 -->
...
export default {
  ...
  methods: {
    ...
    // 保存病例
    handelSaveCareHistory() {
      if (!this.careHistory.regId) {
        this.msgError('请选择挂号患者')
        return
      }
      this.careHistory.caseDate = this.moment(this.careHistory.caseDateObj).format('YYYY-MM-DD')
