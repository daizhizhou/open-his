 ...
 * @description
 * @project Open-His
 * @date 2022年07月27日 下午 08:45
 */
@RestController
@RequestMapping("doctor/scheduling")
public class SchedulingController extends HystrixSupport {
    @Reference
    ...
    public AjaxResult queryUsersNeedScheduling(Long deptId) {
        List<SystemUser> users = this.systemUserService.querySchedulingUsers(null, deptId);
    ...
    @HystrixCommand
    public AjaxResult queryScheduling(Scheduling schedulingQueryDto) {
        //根据部门ID用户ID查询用户信息 如果用户ID和部门ID都为空，那么就查询所有要排班的用户信息
        List<SystemUser> users = systemUserService.querySchedulingUsers(schedulingQueryDto.getUserId(), schedulingQueryDto.getDeptId());
    ...
    @HystrixCommand
    public AjaxResult queryMyScheduling(Scheduling schedulingQueryDto) {
        //根据部门ID用户ID查询用户信息 如果用户ID和部门ID都为空，那么就查询所有要排班的用户信息
        List<SystemUser> users = systemUserService.querySchedulingUsers(Long.valueOf(ShiroSecurityUtil.getCurrentSimpleUser().getUserId().toString()), schedulingQueryDto.getDeptId());
        return getSchedulingAjaxResult(schedulingQueryDto, users);
    }
     ...
     * @return
     */
    private AjaxResult getSchedulingAjaxResult(Scheduling schedulingQueryDto, List<SystemUser> users) {
        //得到当前时间
        ...
        //存放页面需要的数据的对象
        List<Scheduling> schedulingDtos = new ArrayList<>();
        //根据用户来循环
        for (SystemUser user : users) {
            Scheduling schedulingDto1 = new Scheduling(user.getUserId(), user.getDeptId(), "1", initMap(beginTime));
            Scheduling schedulingDto2 = new Scheduling(user.getUserId(), user.getDeptId(), "2", initMap(beginTime));
            Scheduling schedulingDto3 = new Scheduling(user.getUserId(), user.getDeptId(), "3", initMap(beginTime));
            //一个用户的循环放三条数据
            ...
            schedulingDto1.setSchedulingTypeList(schedulingDto1.getRecord().values());
            schedulingDto2.setSchedulingTypeList(schedulingDto2.getRecord().values());
            schedulingDto3.setSchedulingTypeList(schedulingDto3.getRecord().values());
        }
        //组装返回的对象
...
////    @HystrixCommand
//    public AjaxResult saveScheduling(@RequestBody SchedulingForm schedulingFormDto){
//        schedulingFormDto.setSimpleUser(ShiroSecurityUtil.getCurrentSimpleUser());
//        return AjaxResult.toAjax(this.schedulingService.saveScheduling(schedulingFormDto));
//    }

    @PostMapping("saveScheduling")
    @HystrixCommand
    public AjaxResult saveScheduling(@RequestBody String json){
        final SchedulingForm schedulingFormDto = JSON.parseObject(json, SchedulingForm.class);
        schedulingFormDto.setSimpleUser(ShiroSecurityUtil.getCurrentSimpleUser());
        return AjaxResult.toAjax(this.schedulingService.saveScheduling(schedulingFormDto));
    }
